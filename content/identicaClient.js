...
var identicaClient = {};
identicaClient.oauthConsumerSecret = "cc0455ac9663c77b8720c171ccd58c24";
identicaClient.oauthConsumerKey = "5e0d329e843b8bc228f413d59260aad9";
...
identicaClient.requestAccessToken = function(requestToken, pin, doNext, nextParams) {
  var httpMethod = "POST";
  var accessTokenUrl = "http://socialno.de/api/oauth/access_token";
  var parameters = [];
...
identicaClient.authenticate = function (doNext, nextParams) {
  ...
  var httpMethod = "POST";
  var requestTokenUrl = "http://socialno.de/api/oauth/request_token";
  var authorizationUrl = "http://socialno.de/api/oauth/authorize";
  var callback = "oob";
  var parameters = [];
  ...
  req.onreadystatechange = function () {
    if (req.readyState == 4) {
      ...
      switch(req.status) {
        ...
        var pin = prompt("Enter PIN from Socialnode:");
        identicaClient.requestAccessToken(oauth_token, pin, doNext, nextParams);
        break;
        default:
          alert("Failed to authenticate with Socialnode. Reponse was " + req.status);
      }
    }
  }
  req.send(null);
...
identicaClient.identicaUpdates = function() {
  ...
  var httpMethod = "GET";
  var homeUrl = "http://socialno.de/api/statuses/friends_timeline.json?since=" + encodeURIComponent(new Date(milliseconds).toUTCString());
      ...
      if(req.status == 200) {
        ...
        for (var i = 0; i < statuses.length; i++) {
          ...
          try {
            ...
            firestatus.cons.logStringMessage("Error decoding socialnode update: " +
                                             status.text);
            text = status.text;
          }
        ...
        firestatus.cons.logStringMessage("Error getting Socialnode updates. " +
...
identicaClient.sendStatusUpdateIdentica = function (statusText) {
  ...
  var httpMethod = 'POST';
  var postUrl = 'http://socialno.de/api/statuses/update.json';
  ...
  req.onreadystatechange = function () {
    if (req.readyState == 4) {
      ...
      if (req.status != 200)
        alert("Failed to update Socialnode. Response was " + req.status + ": " + req.responseText);
    }
  };
